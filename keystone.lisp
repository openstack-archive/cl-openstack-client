...
(defmethod service-url ((connection connection-v2) service)
...
(defmacro def-rest-method (name lambda-list options)
  "A convenience wrapper around request-resource.

NAME is the name of the method. LAMBDA-LIST is a method lambda list.

OPTIONS is in the form of a PLIST and can contain URI METHOD HEADERS
DOCUMENTATION CONTENT and PARSER values.
...
METHOD this is the HTTP method type, it can be :get :post :put :delete
or :patch.

DOCUMENTATION a documentation string that will be assigned to the
method.

CONTENT this form is evaluated with in a PROGN inside the method.

PARSER this form should be defined as a function that takes one
argument, a stream returned from the HTTP request.  The parser form
will be evaluated within the context of the method and have access to
local variables.
"
  (let ((uri (or (cadr (assoc :uri options))
                 (error ":URI is required.")))
        (method (cadr (assoc :method options)))
        (headers (cadr (assoc :headers options)))
        (documentation (or (cadr (assoc :documentation options)) ""))
        ;; Ensure that the content form is a list of lists so that
        ;; expansion will work.
        (content (let ((content (cadr (assoc :content options))))
                   (if (listp (car content))
                       content
                       (list content))))
        ;; Ensure that the parser is a lambda expression.
        (parser (let ((parser (cadr (assoc :parser options))))
                  (cond ((eql 'lambda (car parser))
                         (cdr parser))
                        ((null parser)
                         nil)
                        (t (error "Parsers must be defined in the form (lambda (stream) ...)"))))))
    (with-gensyms (stream lparser)
      `(defmethod ,name ,lambda-list
         ,documentation
         (flet (,@(when parser `((,lparser ,@parser))))
           (let* ((*resource* ,(let ((res (car lambda-list)))
                                 (if (listp res) (car res) res)))
                  (,stream
                    (request-resource
                     (format nil "~a/~a"
                             (service-url (resource-connection *resource*) "identity")
...
;; XXX (RS) this may no longer be required, instead the connection
;; could be directly passed into the request-resource function.
(defvar *resource* nil
  "A dynamic variable used to pass a resource into the
  request-resource method implicitly.")

(defun request-resource (uri &key method additional-headers content)
  "Must be called with a bound *RESOURCE* to get a valid connection from."
  (multiple-value-bind (body status-code headers uri stream must-close reason-phrase)
      (http-request uri
                    :method method
                    :content-type "application/json"
...
(def-rest-method list-users ((tenant tenant-v2))
    ((:documentation "List all the users for tenant.")
     ...
     (:method :get)
     (:parser (lambda (stream)
                (decode-resource-list (assoc* :users (decode-json stream))
                                      tenant
                                      'user-v2)))))
...
(def-rest-generic get-user (connection user)
  (:documentation "Gets information for a specified user. ")
  (:method (connection user)
      ((:uri "/users/{user}")
       (:method :get)
       (:parser (lambda (stream)
                  (decode-resource (assoc* :user (decode-json stream))
                                   connection
                                   'user-v2))))))
...
(defgeneric delete-user (resource &optional username))

(def-rest-method delete-user ((connection connection-v2) &optional user-id)
  ((:documentation "Delete a user.")
   (:uri "/users/{user-id}")
   (:method :delete)))

(def-rest-method delete-user ((user user-v2) &optional username)
  ((:documentation "Delete a user.")
   (:uri "/users/{user}")
   (:method :delete)
   (:content (when username
               (error "Don't know which user to delete ~A or ~A?" user username)))))
...
(def-rest-method list-roles ((connection connection-v2))
    ((:documentation "List roles.")
     ...
     (:method :get)
     (:parser (lambda (stream)
                (decode-resource-list (assoc* :roles (decode-json stream))
                                      connection
                                      'role-v2)))))
...
tenant roles.")
   ...
   (:method :get)
   (:parser (lambda (stream)
              (decode-resource (assoc* :roles (decode-json stream))
                               user
                               'role-v2)))))
